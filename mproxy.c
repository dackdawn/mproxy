...
/*       p1      p2  p3   */
/*CONNECT tiny.cc:443/ltetp.tv189.com/ HTTP/1.1*/
    		    if(_p2)		//有端口
    		    {
    		        char s_port[10];
    		        bzero(s_port,10);

    		        strncpy(remote_host,_p1+1,(int)(_p2  - _p1) - 1); //拷贝Host的值
					remote_host[(int)(_p2  - _p1) - 1] = '\0';
    		        strncpy(s_port,_p2+1,(int) (_p3 - _p2) -1);  //拷贝端口值
					s_port[(int) (_p3 - _p2) -1] = '\0';
    		        remote_port = atoi(s_port);                   //同上

    		    } else  //无端口
    		    {
    		        strncpy(remote_host,_p1+1,(int)(_p3  - _p1) -1);
					remote_host[(int)(_p3  - _p1) - 1] = '\0';
    		        remote_port = 80;
    		    }
...
/*-------------------------http请求-------------------------------------*/
    		...
    		char * p = strstr(header,"X-Online-Host:");
    		if(!p) //找不到X-Online-Host
    		{
    		...
    		char * p2 = strchr(p + 14,':'); /* 14是指'X-Online-Host:'的长度, p2后的是端口*/
/*		p                     p2  p1
	....X-Online-Host: tiny.cc:443\r\n*/
    		if(p2 && p2 < p1)  //存在端口且合理
    		{
    		    int p_len = (int)(p1 - p2 -1); //端口的长度
				char *s_port = (char *)malloc(10 * sizeof(char));
    		    strncpy(s_port,p2+1,p_len);
    		    s_port[p_len] = '\0';
    		    remote_port = atoi(s_port);

    		    int h_len = (int)(p2 - p - 15); //Host长度
    		    strncpy(remote_host,p + 15  ,h_len); //拷贝Host的值
    		    remote_host[h_len] = '\0';
    		} else  //端口不存在
    		{   
    		    int h_len = (int)(p1 - p - 15); //Host长度
    		    strncpy(remote_host,p + 5 + 1,h_len); //拷贝Host的值
    		    remote_host[h_len] = '\0';
    		    remote_port = 80;
    		}
		} else		//有-r标记
...
/*-------------------------CONNECT请求--------------------------------------*/
    	if(_p && __p)		//CONNECT请求且找到了自定义Host
    	{
    	    char * _p1 = strchr(_p,' ');
    	    char * _p2 = strchr(_p1 + 1,':');
    	    char * _p3 = strchr(_p1 + 1,'\r');
/*__p                                                   _p   _p1     _p2  _p3      */
/*  CONNECT tiny.cc:443/ltetp.tv189.com/ HTTP/1.1..../r/nLbxx: tiny.cc:443/r/n*/
    	    if(_p2) //端口存在
    	    {
    	        char s_port[10];
    	        memset(s_port, 0, 10);
...
/*------------------------http请求-------------------------------*/
    	char * p = strstr(header,m_s);   //找自定义Host
    	if(!p)		//找不到自定义Host
    	{
	    	if(r_flag)		//有-m标记
	    	{
		...
		int m_l = strlen(m_s);
    	char * p2 = strchr(p + m_l,':'); /* m_l是指类似自定义'Lbxx:'的长度，找端口位置 */
/*             p            p2  p1
          (...)Lbxx: tiny.cc:443/r/n*/
    	if(p2 && p2 < p1)		//存在端口且合理
    	{
...
void handle_client(int client_sock, struct sockaddr_in client_addr)
{
    ...
    if(strlen(remote_host) == 0) /* 未指定远端主机名称从http 请求 HOST 字段中获取 */
    {
        ...
        } else 
        {
            ...
            if(extract_host(header_buffer) < 0) //获取Host和端口
            {
...
void rewrite_header()
{
    ...
    int len = strlen(header_buffer);
/*              p                   p1                                p2 p5                p0
            GET http://www.baidu.com/s?tn=80035161_2_dg&wd=123/ashfkj/   HTTP/1.1\r\n(....)*/
    if(p)		//存在http://
    {
        char * p1 = strchr(p + 7,'/');
        if(p1 && (p5 > p1)) //[uri]存在且合理
        {
            //转换url到 path
    ...
    /*不存在"http://",好像就不处理URL转path了*/
/*   
    if(m_flag)		//存在-m标记，不明白为什么要覆盖掉Host，难道本来是无法联网的？
    {			//而且只有http要覆盖，https不用覆盖，真不明白，待研究
				//覆盖的方法太极端，下次找时间修改一下
				//所以暂时不执行此步骤，如发现无法联网，把607行到627行的注释去掉，也就是if(m_flag)这个语句
